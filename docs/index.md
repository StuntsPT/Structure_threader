...
Neural ADMIXTURE has optional support for the [PLINK 2](https://www.cog-genomics.org/plink/2.0/) (`.pgen`) binary format, but [Pgenlib](https://pypi.org/project/Pgenlib/) is required (installed automatically as a dependency only when installed via `pip` with binaries enabled).  
...
* Source distribution with platform binaries for wrapped programs - [Structure_threader on PyPI](https://pypi.org/project/structure_threader/)
    * You can easily install *Structure_threader* with binaries by issuing the command `ST_INSTALL_BINARIES=True pip install structure_threader`
...
Binaries for [Structure](https://web.stanford.edu/group/pritchardlab/structure.html), [fastStructure](https://github.com/rajanil/fastStructure) and [MavericK](https://github.com/bobverity/MavericK) are distributed in the PyPI hosted version.
