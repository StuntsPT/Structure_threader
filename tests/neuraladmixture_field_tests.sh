...
${structure_threader_exec} run -i "${git_dir}/tests/smalldata/BigTestData.bed" -o ~/results_nad -nad "${str_bin}" -K 4 --exec_mode train --nad_seed 42 -t 1 --ind "${git_dir}/tests/smalldata/indfile.txt"

echo -e "${LightGreen}Neural ADMIXTURE Train 'Field test' ran successfully on the \`.bed\` file. Yay!${NoColor}"

#${structure_threader_exec} run -i "${git_dir}/tests/smalldata/BigTestData.bed" -o ~/results_nad -nad "${str_bin}" -K 4 --exec_mode infer -t 1 --ind "${git_dir}/tests/smalldata/indfile.txt"
